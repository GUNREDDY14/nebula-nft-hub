...
export function AuctionCard({
  ...
  // ---------------------------
  // MINIMUM BID CALC (fixed)
  // bigint → string → formatEther
  // ---------------------------
  let minBid: string;

  if (highestBid > 0n) {
    const next = highestBid + BigInt(ethers.utils.parseEther("0.001").toString());
    minBid = ethers.utils.formatEther(next.toString());
  } else {
    minBid = ethers.utils.formatEther(startingPrice.toString());
  }
  ...
  return (
    <Card className="overflow-hidden border-border/30 bg-gradient-to-br from-secondary/50 to-card">
      <div className="relative aspect-square overflow-hidden bg-secondary/30">
        ...
        <div
          className={`absolute top-3 right-3 px-3 py-1.5 rounded-lg backdrop-blur-sm flex items-center gap-2 ${
            isEnded ? 'bg-destructive/90' : 'bg-primary/90'
          }`}
        >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <p className="font-semibold">
              {ethers.utils.formatEther(startingPrice.toString())} ETH
            </p>
          </div>
        ...
        {highestBidder &&
          highestBidder !== ethers.constants.AddressZero && (
            <p className="text-xs text-muted-foreground">
              Highest Bidder: {highestBidder.slice(0, 6)}...
              {highestBidder.slice(-4)}
            </p>
          )}
      </CardContent>
      ...
      <CardFooter className="p-4 pt-0 flex-col gap-3">
        ...
        {isEnded && (
          <Button className="w-full" onClick={onEnd} disabled={loading}>
            {loading ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : null}
            End Auction
